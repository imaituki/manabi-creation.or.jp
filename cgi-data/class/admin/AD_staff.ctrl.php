...
class AD_staff {
	...
	// 主テーブル
	var $_CtrTable   = "mst_master_staff";
	var $_CtrTablePk = "id_master_staff";

	// コントロール機能（ログ用）
	var $_CtrLogName = "総合スタッフ";
	...
	function insert( $arrVal, $arrSql = null ) {
		...
		$arrVal["password"]    = fn_encrypt( $arrVal["password"], _ADMINCRYPTKEY );
	...
	// 内  容：表示切り替え
	//-------------------------------------------------------
	function changeValid( $id, $flg ) {

		// 初期化
		$res = false;

		// 切り替え処理
		$res = $this->_DBconn->update( $this->_CtrTable, array( "valid_flg" => $flg ), null, $this->_CtrTablePk . " = ?", array( $id ) );
	...
	function login( $account, $password ) {
		...
		if( empty( $res["message"]["ng"] ) ) {
			...
			// アカウント・パスワードチェック
			$creation_kit = array(
					"select" => "id_master_staff, account, password, name ",
					"from"   => $this->_CtrTable,
					"where"  => "valid_flg = 1 AND delete_flg = 0 AND account = ?",
					"bind"   => array( $account )
				);
			
			// アカウントデータ取得
			$mst_master_staff = $this->_DBconn->selectCtrl( $creation_kit, array( "fetch" => _DB_FETCH ) );
			
			// パスワードチェック
			$mst_master_staff["password"] = ( !empty( $mst_master_staff["password"] ) ) ? fn_decrypt( $mst_master_staff["password"], _ADMINCRYPTKEY ) : null;
			
			// ログインチェック
			if( $mst_master_staff["id_master_staff"] != "" && $mst_master_staff["password"] === $password ) {
				
				// SESSION ID
				$mst_master_staff["ssid"] = session_id();
				
				// パスワード削除（Cookie保存しない）
				unset( $mst_master_staff["password"] );
				
				// クッキー保管期間（180日間）
				$cookie_limit = time() + 60 * 60 * 24 * 180;
				
				// クッキー設定
				foreach( $mst_master_staff as $key => $val ) {
					...
					// 設定
					if( is_array( $val ) ) {
						@setcookie( "ad_" . $key , fn_encrypt( serialize( $val ), _ADMINCRYPTKEY ), $cookie_limit, "/" );
					} else {
						@setcookie( "ad_" . $key , fn_encrypt( $val, _ADMINCRYPTKEY ), $cookie_limit, "/" );
					}
