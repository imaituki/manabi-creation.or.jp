...
class AD_toys_info {
	...
	// 主テーブル
	var $_CtrTable   = "t_toys_info";
	var $_CtrTablePk = "id_toys_info";

	// コントロール機能（ログ用）
	var $_CtrLogName = "おもちゃ";
	...
	function check( &$arrVal, $mode ) {
		...
		// チェックエントリー
		$objInputCheck->entryData( "カテゴリ"    , "id_toys_info_category", $arrVal["id_toys_info_category"], array( "CHECK_EMPTY" ), null, null );
		$objInputCheck->entryData( "名称"        , "name"                 , $arrVal["name"]                 , array( "CHECK_EMPTY", "CHECK_MIN_MAX_LEN" ), 0, 255 );
		...
		if( ( strcmp( $mode, "update" ) == 0 ) ) {
			$objInputCheck->entryData( "おもちゃID", "all", $arrVal["id_toys_info"], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
	...
	function update( $arrVal, $arrSql = null ) {
		...
		$bind = array( $arrVal["id_toys_info"] );
