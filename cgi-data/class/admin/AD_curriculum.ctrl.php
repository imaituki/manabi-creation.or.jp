...
class AD_curriculum {
	...
	function check( $arrVal, $mode ) {
		...
		// チェックエントリー
		$objInputCheck->entryData( "メニュー名"    , "menu"       , $arrVal["menu"]       , array( "CHECK_EMPTY", "CHECK_MIN_MAX_LEN" ), 0, 255 );
		$objInputCheck->entryData( "カリキュラム名", "name"       , $arrVal["name"]       , array( "CHECK_EMPTY", "CHECK_MIN_MAX_LEN" ), 0, 255 );
		$objInputCheck->entryData( "内容"          , "comment"    , $arrVal["comment"]    , array( "CHECK_EMPTY" ), null, null );
		$objInputCheck->entryData( "公開／非公開"  , "display_flg", $arrVal["display_flg"], array( "CHECK_EMPTY", "CHECK_MIN_MAX_NUM" ), 0, 1 );
		
		// チェックエントリー（UPDATE時）
		if( ( strcmp( $mode, "update" ) == 0 ) ) {
			$objInputCheck->entryData( "カリキュラムID", "all", $arrVal[$this->_CtrTablePk], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
