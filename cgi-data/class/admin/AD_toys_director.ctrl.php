...
class AD_toys_director {
	...
	// 主テーブル
	var $_CtrTable   = "mst_toys_director";
	var $_CtrTablePk = "id_toys_director";

	// コントロール機能（ログ用）
	var $_CtrLogName = "おもちゃディレクタ";
	...
	function check( $arrVal, $mode ) {
		...
		// チェックエントリー
		$objInputCheck->entryData( "名前"        , "name"       , $arrVal["name"]       , array( "CHECK_EMPTY", "CHECK_MIN_MAX_LEN" ), 0, 255 );
		$objInputCheck->entryData( "表示／非表示", "display_flg", $arrVal["display_flg"], array( "CHECK_EMPTY", "CHECK_MIN_MAX_NUM" ), 0, 1 );
		
		// チェックエントリー（UPDATE時）
		if( ( strcmp( $mode, "update" ) == 0 ) ) {
			$objInputCheck->entryData( "おもちゃディレクタID", "all", $arrVal[$this->_CtrTablePk], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
