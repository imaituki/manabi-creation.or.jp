                ...
                <div class="box">
    				...
    				<h3>7.お問い合わせ</h3>
        				...
        				<p><strong>一般社団法人 日本学びクリエイション</strong><br>
                            住所：{html_select_ken pre="1" selected=$displaySchool.prefecture}{$displaySchool.address1}{$displaySchool.address2}<br>
							{if $displaySchool.tel|default:"" != NULL}電話番号：<span class="tel" data-tel="{$displaySchool.tel}"><a href="tel:{$displaySchool.tel}">{$displaySchool.tel}</a></span><br>{/if}
							{if $displaySchool.mail|default:"" != NULL}メール：<a href="mailto:{$displaySchool.mail}">{$displaySchool.mail}</a>{/if}
        				</p>
    			</div>
            </div>
        </div>
