			<form class="form-horizontal" action="./{if $mode == 'edit'}update{else}insert{/if}.php" method="post" enctype="multipart/form-data">
				<div class="ibox-content">
					...
					<div class="form-group required">
						...
						<div class="col-sm-6">
							{if $message.ng.id_information_category|default:"" != NULL}<p class="error">{$message.ng.id_information_category}</p>{/if}
							<select name="id_information_category" class="form-control">
								<option value="">選択してください</option>
								{html_options options=$OptionInformationCategory selected=$arr_post.id_information_category}
							</select>
						</div>
					</div>
					<div class="hr-line-dashed"></div>
					...
					<div class="hr-line-dashed"></div>
					{if $mode == 'edit'}<input type="hidden" name="id_school_information" value="{$arr_post.id_school_information}" />{/if}
